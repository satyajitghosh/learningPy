# This example shows a function that is wrapped with two deorators.
...
class LoggingDecorator(AbstractDecorator):
    def execute(self,upper_bound:int)->int:
        print(f"Starting:{self._component.__class__.__name__}")
        value = self._component.execute(upper_bound)
        print(f"Completed:{self._component.__class__.__name__}")


def main():
    component = Corecomponent()
    benchamerkeddecorator = BenchmarkDecorator(component)
    loggedandbenchmarkeddecorator = LoggingDecorator(benchamerkeddecorator)
    loggedandbenchmarkeddecorator.execute(10000)
